...
src/
├── app.ts              # 应用入口（黑暗模式注入）
├── app.config.ts        # 应用配置（tabBar、39 个页面路由）
├── app.css             # 全局样式 + CSS 变量 + 黑暗模式
├── pages/              # 55 个页面（tabBar 5 页 + 50 个子页面）
...
70. 点赞动画（❤️ 弹出特效）
71. 内容展开 / 收起（长文截断）
72. 图片九宫格预览（previewImage）
73. 空状态统一设计（EmptyState）
74. 页面加载态（loading 提示）
75. 下拉刷新动画（RefreshControl）
76. 上拉触底加载（触底自动分页）
77. Toast 轻提示（操作反馈）
78. Modal 确认弹窗（删除 / 支付）
79. 复制到剪贴板（邀请链接 / 项目地址）
...
**组件体系**
91. FeedCard 动态卡片（复用 8 处）
92. UserAvatar 头像（三种尺寸 + VIP 徽章）
93. ActionBar 互动栏（赞 / 评 / 转）
94. CommentList 评论列表（子评论嵌套）
95. NavBar 自定义导航（返回 / 右侧操作）
96. SearchBar 搜索栏（自动聚焦）
97. TopicTag 话题标签（可点击跳转）
98. EmptyState 空状态（图标 + 引导按钮）
99. 全局 CSS 变量设计系统（颜色 / 间距 / 圆角）
100. 双端构建（H5 + 微信小程序同一套代码）
